...
export const annotationCurriculumStudent = () => {
  ...
  const addAnnotationLesson = (canvas: any, propImage: any, item: any, bindAll: boolean, event: any, group: any) => {
  ...
  const processLessonImage = (
    ...
    if (annotation && annotation.length) {
      const data = processAnnotationLesson(canvas, propImage, containerRef, isShowWhiteBoard, true, event, Group);
    ...
    if (!isImgProcessing.value) {
      ...
      if (zoomRatio.value > MIN_ZOOM_RATIO) {
        canvas.zoomToPoint(point, canvasZoomRatio + (zoomRatio.value - MIN_ZOOM_RATIO) * canvasZoomRatio);
      }
      if (imgCoords.value) {
