...
export function betweenDate(
  min: moment.MomentInput = moment(),
  max: moment.MomentInput = moment()
) {
  return (current: any) => {
    return moment(current, "YYYY/MM/DD").isBetween(
...
export class GLGlobal {
  ...
  public static authorityUrl = () =>
    GLGlobal.processEnv().VUE_APP_AUTHORITY_URL;
  public static authorityClientId = () =>
    GLGlobal.processEnv().VUE_APP_AUTHORITY_CLIENT_ID;
  public static authorityClientSecret = () =>
    GLGlobal.processEnv().VUE_APP_AUTHORITY_CLIENT_SECRET;
...
export class GLUtil {
  ...
  static setLocaleCookie(language: string) {
    ...
    GLUtil.cookies.set("GrapeSeed.Culture", `c=${language}|uic=${language}`, {
      domain:
        process.env.portalsDomain ||
        process.env.VUE_APP_AUTHORITY_URL.replace(
          /.*?\.(.*?\.[com|net])/,
          "$1"
        ),
    });
  ...
  static pathStringify<T = any>(
    pathname: string,
    obj: T,
    opts?: { encode: (value: any) => any }
  ): string {
