...
import {
  IGLRequestErrorHandler,
  GLRequestErrorHandler,
  ...
  constructor(offset?: number | object, limit?: number, searchParams?: object) {
    ...
    params &&
      Object.getOwnPropertyNames(params).map(
        (key) =>
          !Reflect.hasMetadata(reqIgnoreMetadataKey, params, key) &&
...
export interface ITGLRequest<TRequest, TResponse> extends IGLRequest {
  get<TResp = TResponse, TReq = object | TRequest>(
    url: string,
    data?: TReq,
    routeData?: object
  ): Promise<TResp>;
  ...
  merge<TResp = TResponse, TReq = GLMergeDoc[] | any>(
    url: string,
    data?: TReq,
    routeData?: object
  ): Promise<TResp>;
  ...
  getBlob<TResp = TResponse, TReq = object | TRequest>(
    url: string,
    data?: TReq,
    routeData?: object
  ): Promise<TResp>;
...
export class GLRequestProxy<TRequest, TResponse extends GLResponse>
  implements ITGLRequest<TRequest, TResponse> {
      ...
      Headers: [
        {
          Authorization:
            "Bearer " +
            (GLGlobal.loginInfo().access_token || AuthService.accessToken),
          "x-gl-sub":
          ...
          "x-gl-countrycode": enabled
            ? localStorage.getItem(CONSTS.COUNTRY_CODE)
            : null,
        },
      ],
  ...
  processResponse<TResp = TResponse>(
    ...
    this.trySSOSignin();
    let request = func(
      param.Prefix && !param.Url?.startsWith("http")
    ...
    param.RequestIsMultipart &&
      request
        .field((param.Data as any) as { [fieldName: string]: string | Blob })
        .unset("Content-Type");
    ...
    return new Promise((resolve, reject) => {
      if (!isLocalEnvironment()) {
        request = request.use(
          corsProxy({ type: param.RequstIsBlob ? "blob" : "json" })
        );
      }
      request.end((error, response) => {
        ...
        response &&
          ...
          resolve(
            param.RequstIsBlob
              ? this.getBlobResult(response)
              : this.getResultByHeader(response)
          );
      });
    });
  }
  ...
  buildRouterParam(url: any, data?: any, routeData?: any) {
    if (
      (url as string).lastIndexOf("}") < 0 ||
      (!routeData && !data) ||
      (data instanceof Array && !routeData)
    ) {
      return url;
    }
    ...
    return this.processResponse<TResp>(
      ...
      this.buildData([
        this.buildQueryParam(
          this.buildRouterParam(url, data, routeData),
          data,
          true
        ),
        data,
  ...
  download<TResp = TResponse, TReq = object | TRequest>({
    ...
    return this.processResponse<TResp>(
      method ? (request as any)[method] : request.get,
      requestParams
    );
  }
  agent(): RequestAgent {
