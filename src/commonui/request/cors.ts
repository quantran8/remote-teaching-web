export class CorsProvider {
  static iframe: any;
  static requests: { [key: string]: any } = {};
  static ready = false;
  ...
  static onMessage(evt: any) {
    if (
      !CorsProvider.iframe ||
      evt.source !== CorsProvider.iframe.contentWindow
    ) {
      return;
    }
    try {
  ...
  static buildRequestProxy() {
    CorsProvider.iframe ||
      window.addEventListener("message", CorsProvider.onMessage);
    CorsProvider.iframe =
      CorsProvider.iframe ||
      CorsProvider.buildFrame(process.env.VUE_APP_API_PREFIX);
  }
}
