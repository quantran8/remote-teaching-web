...
export class AgoraWebRtcWrapper extends EventEmitter implements IWebRTCWrapper {
  ...
  async join(option: any): Promise<any> {
    ...
    // Logger.debug(`web#join ${JSON.stringify(option)}`)
  ...
  async openCamera(option?: CameraOption): Promise<any> {
    ...
    if (!option) {
      ...
      Logger.info(
        `[agora-web] create camera [${cameraId}], option: ${JSON.stringify(
          option
        )} success`
      );
    }
  ...
  async closeCamera() {
    Logger.info("[agora-web] invoke close#openCamera");
    if (this.cameraTrack) {
      try {
        ...
        if (this.cameraTrack) {
          ...
          Logger.info(`[agora-web] close camera [${trackId}] success`);
          this.cameraTrack = undefined;
        }
      } catch (err) {
        if (this.cameraTrack) {
  ...
  async changeCamera(deviceId: string): Promise<any> {
    Logger.info("[agora-web] invoke close#changeCamera");
    if (this.cameraTrack) {
      await this.cameraTrack.setDevice(deviceId);
      await this.agoraWebSdk.checkVideoTrackIsActive(
        this.cameraTrack as ILocalVideoTrack
      );
      Logger.info(`[agora-web] changeCamera by deviceId: ${deviceId} success`);
    } else {
  ...
  async openMicrophone(option?: MicrophoneOption): Promise<any> {
    Logger.info("[agora-web] invoke web#openMicrophone");
    if (!option) {
      ...
      Logger.info(
        `[agora-web] create audio track [${this.microphoneTrack.getTrackId()}] success`
      );
    } else {
      this.microphoneTrack = await this.agoraWebSdk.createMicrophoneAudioTrack({
      ...
      Logger.info(
        `[agora-web] create audio track with  by deviceId: ${
          option.deviceId
        } [${this.microphoneTrack.getTrackId()}] success`
      );
    }
    if (this.microphoneTrack) {
      this.microphoneTrack.stop();
      Logger.info(
        `[agora-web] create audio track stop playback [${this.microphoneTrack.getTrackId()}] success`
      );
    }
    if (this.joined && this.publishedAudio) {
      await this.client.publish([this.microphoneTrack]);
      Logger.info(
        `[agora-web] publish audio track [${this.microphoneTrack.getTrackId()}] success`
      );
    }
  }
  ...
  async closeMicrophone() {
    Logger.info("[agora-web] close microphone");
    if (this.microphoneTrack) {
      try {
        ...
        if (this.microphoneTrack) {
          ...
          Logger.info("[agora-web] close microphone success");
          this.microphoneTrack = undefined;
        }
      } catch (err) {
        if (this.microphoneTrack) {
  ...
  async changeMicrophone(deviceId: string): Promise<any> {
    if (this.microphoneTrack) {
      ...
      await this.agoraWebSdk.checkAudioTrackIsActive(
        this.microphoneTrack as ILocalAudioTrack
      );
    } else {
  ...
  private async unpublishTrack(track: ILocalTrack) {
    ...
    if (this.cameraTrack && this.cameraTrack.getTrackId() === trackId) {
      ...
      Logger.info(`[agora-web] unpublish camera track [${trackId}] success`);
    }
    if (this.microphoneTrack && this.microphoneTrack.getTrackId() === trackId) {
      await this.client.unpublish([this.microphoneTrack]);
      Logger.info(
        `[agora-web] unpublish microphone track [${trackId}] success`
      );
    }
  ...
  async unpublish(): Promise<any> {
    if (this.cameraTrack) {
      ...
      if (idx !== -1) {
        await this.client.unpublish([this.cameraTrack]);
        Logger.info("[agora-web] unpublish camera track success");
        this.publishedTrackIds.splice(idx, 1);
      }
    }
    if (this.microphoneTrack) {
      ...
      if (idx !== -1) {
        await this.client.unpublish([this.microphoneTrack]);
        Logger.info("[agora-web] unpublish microphone track success");
        this.publishedTrackIds.splice(idx, 1);
      }
    }
  }
  ...
  async changeTestCamera(deviceId: string): Promise<any> {
    if (this.cameraTestTrack) {
      ...
      await this.agoraWebSdk.checkVideoTrackIsActive(
        this.cameraTestTrack as ILocalVideoTrack
      );
    } else {
  ...
  async openTestMicrophone(option?: MicrophoneOption): Promise<any> {
    ...
    if (!option) {
      ...
      this.microphoneTestTrack.on("track-ended", () => {
        this.microphoneTestTrack &&
          this.closeMediaTrack(this.microphoneTestTrack);
        this.fire("track-ended", { audio: true });
      });
    } else {
      this.microphoneTestTrack = await this.agoraWebSdk.createMicrophoneAudioTrack(
  ...
  async changeTestMicrophone(deviceId: string): Promise<any> {
    if (this.microphoneTestTrack) {
      ...
      await this.agoraWebSdk.checkAudioTrackIsActive(
        this.microphoneTestTrack as ILocalAudioTrack
      );
    } else {
